...
socket.on('db_return', function (data) {
        ...
        $.ajax({
            ...
            success: function(vals){
                $.each(vals,function(k,v){
                    conversions[k] = {name: v["name"], equation: v["equation"], units: v["units"]};
                })  
    ...
    function makeTextDisplay(){
        $.each(signals, function(k,v){
            var str = "<div class='row-fluid force-fluid' style='font-size:1.2em'><div class='span7 force-fluid'>"+signals[k].name+"</div><div id='signal"+k+"_val' class='span4 text-right force-fluid' style='font-weight:bold'></div><div class='span1 force-fluid'>"+conversions[signals[k].type].units+"</div></div>";
    ...
    function updateTextDisplay(){
        $.each(signals, function(k,v){
            $("#signal"+k+"_val").html(data_converted[k][data_converted[k].length-1][1].toFixed(2));
        });
        var tr = Math.max(0,(end_time - new Date())/1000); //time in seconds
        var tr = tr > 0 ? tr : 0
        var hr = Math.floor(tr/60/60);
        var mi = Math.floor(tr/60 - hr*60);
        var se = Math.floor(tr-hr*60*60-mi*60);
            ...
            $.ajax({
                ...
                success: function(msg){
                    if (start == "start") {
                        ...
                        end_time = (new Date()*1.0) + parseFloat($("#duration").val())*60*1000;
                    }
                }
            })
        }
